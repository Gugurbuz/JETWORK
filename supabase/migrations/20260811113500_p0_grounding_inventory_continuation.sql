...
-- 1) Reserved inventory prefixes allow continuation from a verified last record without semantic search.
-- 2) Message inventory can be enriched deterministically from authoritative EMITS_MESSAGE relations.
-- 3) Knowledge/research turns cannot complete with zero verified source_refs.
-- 4) knowledge_used telemetry is normalized against actual knowledge tool source_refs.
...
as $function$
  ...
  filter_args as (
    select
      raw_prefix,
      (
        raw_prefix = '__jetwork_message_methods__'
        or raw_prefix like '__jetwork_message_methods__|cursor=%'
      ) as relation_mode,
      case
        when raw_prefix like '__jetwork_message_methods__|cursor=%'
          then nullif(substring(raw_prefix from length('__jetwork_message_methods__|cursor=') + 1), '')
        when raw_prefix like '__jetwork_resume__|cursor=%'
          then nullif(substring(raw_prefix from length('__jetwork_resume__|cursor=') + 1), '')
        else null
  ...
  effective_args as (
    select
      ...
      nullif(
        regexp_replace(lower(coalesce(f.effective_prefix, '')), '[^a-z0-9]+', '', 'g'),
        ''
  ...
  candidates as (
    ...
    where o.publication_status = 'published'
      and s.published_status = 'published'
      ...
      and (
        ...
        or (
          f.normalized_prefix is not null
          and regexp_replace(lower(coalesce(o.published_name, ')), '[^a-z0-9]+', '', 'g') like f.normalized_prefix || '%'
        )
        or (
          f.normalized_prefix is not null
          and regexp_replace(lower(split_part(o.canonical_key, ':', 2)), '[^a-z0-9]+', '', 'g') like f.normalized_prefix || '%'
        )
      )
  ),
  deduped as (
  ...
  page_plus_one as (
    ...
    cross join effective_args f
    where f.effective_cursor is null
      or d.canonical_key > f.effective_cursor
  ...
  relation_methods as (
    select distinct
      ...
      upper(split_part(source_canonical_key, ':', 2)) as method_name
    from relation_candidates
    where override_rank = 1
      and source_canonical_key is not null
      and target_canonical_key is not null
  ),
  relation_agg as (
    select
      target_canonical_key,
      string_agg(method_name, ', ' order by method_name) as methods
    from relation_methods
    group by target_canonical_key
  ),
  page_stats as (
    select
    ...
    'items', coalesce((
      select jsonb_agg(
        jsonb_build_object(
          ...
          'name', v.object_name,
          'title', case
            when f.relation_mode and v.object_type = 'message' then
              concat(
      ...
      from visible v
      cross join effective_args f
      left join relation_agg ra
        on ra.target_canonical_key = v.canonical_key
    ), '[]'::jsonb),
...
begin
  ...
  -- Keep the lock order identical to claim_assistant_turn: turn, then conversation.
  select *
...
begin
  if new.knowledge_used is distinct from true then
    return new;
  end if;
...
drop trigger if exists assistant_reasoning_runs_grounded_knowledge_used
  on public.assistant_reasoning_runs;

create trigger assistant_reasoning_runs_grounded_knowledge_used
before insert or update of knowledge_used
on public.assistant_reasoning_runs
for each row
execute function private.normalize_reasoning_knowledge_used();
