-- Preserve the compact resolved/provider conversation state produced by the
-- active controller runtime. The literal source-line provenance migration
-- accidentally initialized clean_state_items to [] and never copied the
-- validated p_state_items payload, causing every successful completion to wipe
-- assistant_conversations.state_items.
--
-- This migration changes no semantic routing or grounding policy. It keeps the
-- existing mechanical/provenance guards intact and restores durable state.
...
declare
  ...
  literal_line_supported boolean;
...
begin
  ...
  -- State has already been compacted/sanitized by the controller runtime. Keep
  -- it only after validating the RPC contract above; do not silently replace it
  -- with an empty array.
  clean_state_items := p_state_items;
  ...
  where m.id = turn_record.message_id
    and m.workspace_id = turn_record.workspace_id
    and m.role = 'user'
  limit 1;
  ...
  if coalesce(current_user_text, '') ~* '(abap|source[[:space:]]*code|kaynak[[:space:]]*kod|implementasyon|implementation|kodunu|kodu)' then
    ...
    where m.workspace_id = turn_record.workspace_id
      ...
      and m.created_at < coalesce(
        (select created_at from public.messages where id = turn_record.message_id limit 1),
        now()
      )
  ...
  if enterprise_grounding_required
    ...
    and not grounding_fail_closed
  then
    raise exception 'ENTERPRISE_GROUNDING_REQUIRED_NO_VERIFIED_SOURCE';
  end if;

  -- Literal enterprise source-code claims remain fail-closed. Every
  -- substantive non-comment code line must exist in at least one cited current
  ...
  loop
    ...
    for code_line in
      select trim(line)
      from regexp_split_to_table(coalesce(code_match[2], ''), E'\n') as line
      where trim(line) <> ''
    loop
      if code_line ~ '^\s*["*]' then
        continue;
      ...
      normalized_line := regexp_replace(lower(code_line), '[[:space:]]+', '', 'g');
      if length(normalized_line) < 3 then
        continue;
      end if;
      ...
      select exists (
        ...
        where coalesce(source_ref ->> 'sourceType', 'knowledge') <> 'web'
          and position(
            normalized_line in regexp_replace(
              lower(coalesce(v.content, '')),
              '[[:space:]]+',
              '',
              'g'
            )
          ) > 0
      ) into literal_line_supported;

      if not coalesce(literal_line_supported, false) then
        raise exception 'UNVERIFIED_LITERAL_SOURCE_CODE_LINE: %', left(code_line, 180);
      end if;
    end loop;
  end loop;
