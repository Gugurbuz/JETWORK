-- Hotfix: claim_trivial_assistant_turn() returns a TABLE column named turn_id.
-- Inside PL/pgSQL, `on conflict (turn_id)` is therefore ambiguous between the
-- output variable and assistant_reasoning_runs.turn_id. Target the unique
-- constraint by name so PostgreSQL never has to resolve that identifier.
