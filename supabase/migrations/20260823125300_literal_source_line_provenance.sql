-- Follow-up to literal source-code grounding: excerpts may omit source lines,
-- but every substantive returned code line must be present in a cited current
-- published knowledge object. This keeps fail-closed provenance without
-- requiring the assistant to reproduce one contiguous source block.
...
begin
  ...
  select coalesce(
    case
      when r.plan ? 'enterpriseGroundingRequired' then (r.plan ->> 'enterpriseGroundingRequired')::boolean
  ...
  loop
    ...
    loop
      ...
      if length(normalized_line) < 3 then continue; end if;
      ...
      select exists (
        ...
        where coalesce(source_ref ->> 'sourceType', 'knowledge') <> 'web'
          and position(normalized_line in regexp_replace(lower(coalesce(v.content, '')), '[[:space:]]+', '', 'g')) > 0
