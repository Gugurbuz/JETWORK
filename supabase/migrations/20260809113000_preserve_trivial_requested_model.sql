-- Preserve the user's explicitly requested model on the durable conversation.
-- The trivial fast path may execute Gemini Pro greetings on Flash Lite for
-- latency, but response-model attribution must not mutate the conversation's
-- requested-model contract. Otherwise the next Pro greeting sees a
-- Pro-vs-Flash-Lite mismatch and unnecessarily falls back to the full core.
