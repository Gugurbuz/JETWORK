-- Normalize every Gemini-facing legacy trivial turn to Gemini 3.8 Flash.
-- Old Gemini ids remain accepted only as stale-client aliases; they are never
-- persisted as the execution model for a newly claimed trivial turn.
...
declare
  ...
  v_execution_model text := case
    when p_model = 'auto' or p_model like 'gemini-%' then 'gemini-3.8-flash'
    else p_model
  end;
...
begin
  ...
  -- Backward-compatible request aliases. All Gemini aliases execute as 3.8.
  if p_model not in (
    'auto',
    'gpt-5.6-sol',
    'gpt-5.6',
  ...
  -- Trivial turns deliberately reuse the active conversation regardless of the
  -- substantive model. Only a prompt-version change invalidates reuse.
  select conversation.*
    into v_conversation
  ...
  if v_claim.outcome = 'claimed' then
    insert into public.assistant_reasoning_runs (
      ...
      v_owner_id,
      v_prompt_id,
      'trivial-fast-path-v10-gemini-38-only',
      'simple_answer',
      'low',
      jsonb_build_object(
