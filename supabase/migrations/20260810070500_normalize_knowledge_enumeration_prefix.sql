-- Normalize technical enumeration prefixes so separator variants match the same catalog objects.
-- Examples: ZCRMCOST, ZCRM_COST, zcrm-cost and zcrm cost all match ZCRM_COST-* message names.
...
as $$
  ...
  filter_args as (
    select nullif(
      regexp_replace(lower(coalesce(trim(p_prefix), '')), '[^a-z0-9]+', '', 'g'),
      ''
...
comment on function public.list_knowledge_catalog_v2(text, text, text, text, integer) is
  'Bounded catalogue enumeration for authenticated workspace members. Prefix matching is case- and separator-insensitive for technical identifiers; project knowledge overrides matching global canonical keys.';
