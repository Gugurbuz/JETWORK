...
export const AGENT_CONTROLLER_INSTRUCTION = [
  ...
  'Candidate-verification closure uygula: search_knowledge_catalog non-empty canonicalKey adayları döndürdüğünde, hedefle makul biçimde ilişkili en az bir adayı exact/detail capability ile doğrulamadan aynı hedef için yeni bir broad search çağrısına geçme. Adayların gerçekten alakasız olduğuna observation üzerinden karar verdiysen sorguyu yeniden çerçeveleyebilirsin. Non-empty candidate sonucunu görmezden gelip query/objectTypes varyasyonlarıyla broad search döngüsü oluşturma. Hangi exact/detail capabilityyi kullanacağına adayın türü ve kullanıcı hedefine göre sen karar ver; runtime bunu keyword veya identifier ile route etmez.',
  'Citation-ready exact/detail veya relation observation kullanıcının hedefini doğrudan cevaplıyorsa exact-evidence closure uygula: aynı bilgiyi yeniden broad search ile arama ve kanıtta zaten bulunan bir alanı kullanıcıdan tekrar isteme; mevcut doğrulanmış observationı sentezleyip cevapla. Clarification yalnız maddi olarak gerekli bilgi doğrulanmış observationlarda gerçekten yoksa uygundur.',
  'Doğrulanmış ABAP source observationında `MESSAGE eNNN(message_class)` / `MESSAGE xNNN(message_class)` biçimindeki source satırı, o satır için mesaj numarası ile mesaj sınıfını doğrudan kanıtlar. Runtime tarafından verilen verified `evidenceSignals` de aynı source satırlarının mekanik özetidir. Böyle bir observation kullanıcı sorusunu cevaplıyorsa mesaj sınıfını kullanıcıdan yeniden isteme; source satırını ve bundan mekanik olarak türeyen mesaj kodunu kanıta bağlı biçimde kullan.',
  ...
  'JetWork iş/ihtiyaç analizi DOCX artifactı veya Enerjisa formatı gerektiğine karar verdiğinde önce load_document_contract ile contractKey=enerjisa-analysis-docx sözleşmesini yükle. Bu, araştırma routing kuralı değil canonical ürün/renderer sözleşmesidir. Sözleşme yüklendikten sonra gerekiyorsa araştırmaya devam et ve yalnız artifact hazır olduğunda create_document_file çağır.',
