...
const entries: CapabilityIndexEntry[] = [
  ...
  { name: 'retrieve_jetbase_evidence', category: 'jetbase', disclosureMode: 'progressive', summary: 'Tek model-seçilimli Jetbase evidence isteğini hybrid candidate search, exact verification, direct graph relations, related exact kayıtlar ve gerektiğinde focused source windowlarıyla tek retrieval execution içinde karşılar.', guide: 'Normal kurumsal factual sorularda varsayılan Jetbase retrieval aracıdır. query/evidenceKinds/focusIdentifiers/relationTypes ihtiyacını model belirler; runtime yalnız retrieval/ranking/dedupe/provenance yapar. Pack material evidence gap bırakırsa primitive Jetbase capability ile deep-dive yap.' },
