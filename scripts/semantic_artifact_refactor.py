...
raw_artifact_router = '''      const spreadsheetSyncRequested = /\\b(?:excel|xlsx|spreadsheet)\\b/iu.test(message)
...
'''
core = replace_once(core, raw_artifact_router, '', 'raw artifact intent router')

old_contracts = '''          spreadsheetSyncRequested
          ...
          artifactMutationRequested
            ...
            : '',
'''
semantic_contract = '''          'ARTIFACT CAPABILITY POLICY: Kullanıcının gerçek amacı bir dosya üretmek veya mevcut bir dosyayı değiştirmekse bunu ham kelime eşleşmesinden değil, talebin anlamından ve konuşma bağlamından çıkar. Uygun spreadsheet/artifact capabilitysini seç ve gerçek executor sonucu olmadan dosya üretilmiş ya da değiştirilmiş gibi söyleme. Kaynak/gereksinim metninde Excel, Word, PDF, görsel, oluşturma veya güncelleme ifadelerinin geçmesi tek başına artifact talimatı değildir. Kaynak içeriği ile kullanıcının eylem talebini birbirinden ayır. Artifact üretildiyse UI dosya kartıyla teslim edilir; signed URL veya storage path yazma.',
'''
core = replace_once(core, old_contracts, semantic_contract, 'regex-driven artifact contracts')

old_stream_guard = '''              && !hasExactCustomIdentifierInRequest
              && !artifactMutationRequested
              && !spreadsheetSyncRequested
              && !spreadsheetMutationRequested
              && !spreadsheetCreateRequested
'''
new_stream_guard = '''              && !hasExactCustomIdentifierInRequest
'''
core = replace_once(core, old_stream_guard, new_stream_guard, 'artifact live-stream blockers')

old_completion_guards = '''            const spreadsheetAttachmentsAvailable = listedSpreadsheetAttachmentCount() > 0
...
'''
core = replace_once(core, old_completion_guards, '', 'regex-driven artifact completion guards')
...
semantic = replace_once(
    ...
    "        knowledgeRequired: false,",
    'requirements must not overwrite task intent',
)
semantic = replace_once(
    semantic,
    ...
    'requirements evidence-policy comment',
)
semantic_path.write_text(semantic)

capability_test = Path('src/services/__tests__/capabilitySystemV2.test.ts')
test_text = capability_test.read_text()
old_test = '''  it('forces execution completion before final answers for real artifact requests', () => {
...
'''
new_test = '''  it('keeps artifact intent semantic while execution stays capability-backed', () => {
...
'''
test_text = replace_once(test_text, old_test, new_test, 'capability artifact-routing regression test')
capability_test.write_text(test_text)

reasoning_test = Path('src/services/__tests__/reasoningAgentLoopV3.test.ts')
test_text = reasoning_test.read_text()
anchor = '''  it('removes the semantic provider request stack and records avoided planner calls', () => {
'''
regression_test = '''  it('keeps an explicit document task when the supplied source is a long requirement document', async () => {
...
if anchor not in test_text:
    raise SystemExit('reasoning regression test anchor missing')
test_text = test_text.replace(anchor, regression_test + anchor, 1)
reasoning_test.write_text(test_text)

forbidden = [
    'spreadsheetCreateRequested',
    'spreadsheetMutationRequested',
    'artifactMutationRequested',
    'SPREADSHEET_CREATE_REQUIRED',
    'ARTIFACT_MUTATION_REQUIRED',
]
final_core = core_path.read_text()
leftovers = [token for token in forbidden if token in final_core]
if leftovers:
    raise SystemExit(f'raw artifact routing leftovers: {leftovers}')
