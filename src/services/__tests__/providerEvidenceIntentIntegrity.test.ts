...
describe('provider, evidence and semantic intent integrity', () => {
  ...
  it('makes the primary LLM the substantive execution decision layer', () => {
    ...
    expect(orchestratorSource).toContain("SEMANTIC_ORCHESTRATOR_VERSION = 'primary-llm-agent-v2-adaptive-work-context'")
    expect(orchestratorSource).toContain('semantic_planner_provider_calls_avoided')
    expect(orchestratorSource).toContain('short_context_bridge_enabled')
    ...
    expect(controllerPolicySource).toContain('Çalışma biçimin reactive "ara-bul-ara-bul" değildir')
