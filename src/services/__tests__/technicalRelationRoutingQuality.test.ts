...
} from '../../../supabase/functions/_shared/technicalRelationRoutingQuality';

const semanticSource = readFileSync(new URL('../../../supabase/functions/_shared/semanticOrchestratorQuality.ts', import.meta.url), 'utf8');
const toolSource = readFileSync(new URL('../../../supabase/functions/_shared/assistantToolsRelationQuality.ts', import.meta.url), 'utf8');
...
describe('generic technical-reference relation routing', () => {
  it.each([
    ...
    expect(detectTechnicalReferenceRelationLookup(message)).toEqual({ technicalReference, targetObjectTypes, relationKind });
  });
  ...
  it('matches exact technical identifier boundaries across identifier families', () => {
  ...
  it('routes technical relations to one generic verified tool and disables catalog enumeration', () => {
    ...
    expect(semanticSource).not.toContain('CHECK_ZTKS ===');
  });
  ...
  it('keeps CHECK_ZTKS only as a regression fixture', () => {
    const fixture = [
