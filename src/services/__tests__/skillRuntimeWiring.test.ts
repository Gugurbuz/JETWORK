...
describe('JetWork skill runtime wiring', () => {
  ...
  it('keeps the same skill executors available to OpenAI and Gemini outside final synthesis-only rounds', () => {
    ...
    expect(implementationSource).toContain('const skillToolsEnabled = !mustSynthesize && !forceEvidenceFinalSynthesis')
    expect(implementationSource).toContain('allowTools: !forceEvidenceFinalSynthesis && (tools.length > 0 || providerWebEnabled || geminiNativeWebPlanned)')
  })
  ...
  it('keeps Gemini provider web semantically independent while final evidence synthesis disables every external capability mechanically', () => {
