...
describe('Gemini answer provider resilience', () => {
  it('fails over from Pro after one transient attempt and retries bounded stable Flash work', () => {
    ...
    expect(source).toContain('const maxAttempts = immediateFailoverCandidate ? 1 : GEMINI_RETRY_DELAYS_MS.length + 1');
