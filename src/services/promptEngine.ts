...
5. Aksiyon: Karar verdiğin stratejiyi uygula (örneğin 'update_document_section' aracını çağır).
`.trim(),
  totInstruction: `
...
`.trim(),
  fewShotLibrary: {
    BA: `
[ÖRNEK ÇIKTI FORMATI - İŞ ANALİZİ DOKÜMANI]
İş Analizi dokümanını mutlaka aşağıdaki başlık yapısına ve formata göre üret:
...
6. SÜREÇ RİSK ANALİZİ
   ...
   6.3. Süreç Etkileri
7. ONAY
   7.1. İş Analizi (Tablo formatında: Analiz Tamamlanma Tarihi, Hazırlayan, Kontrol Tarihi, Kontrol Eden)
   7.2. Değişiklik Kayıtları (Tablo formatında: Tarih, Hazırlayan, Sürüm, Değişiklik Açıklaması)
   7.3. Doküman Onay (Tablo formatında: Tarih, Onaylayan, Görevi, İmza)
   7.4. Referans Dokümanlar (Tablo formatında: Tür, Doküman, Talep Dokümanı, Link)
8. FONKSİYONEL TASARIM DOKÜMANLARI

(Lütfen oluşturduğun dokümanın içeriğini bu numaralandırılmış başlıklara uygun, tablolar ve gerekli kırılımlar ile detaylandırarak doldur. Özellikle tabloları Markdown formatında eksiksiz çiz.)
`.trim(),
    IT: `
