...
export const OrchestratorDecisionSchema = {
  ...
  properties: {
    ...
    reason: {
      type: "string",
      description: "Bu ajanı neden seçtiğinin açıklaması (sistem içi log)."
    },
    messageToUser: {
      type: "string",
      description: "Eğer nextAgent USER veya DONE ise, kullanıcıya gösterilecek doğal dilde, samimi ve açıklayıcı mesaj. Kullanıcıya ne sormak veya söylemek istiyorsan buraya yaz."
    }
  },
...
export class AgentOrchestrator {
  ...
  constructor(callGemini: CallGeminiFunction, initialDocument: DocumentData | null, history: Message[], activeRoles: string[] = ["BA", "IT", "QA", "PO", "UIUX"]) {
  ...
  public async step(
    ...
    let decisionJson = '';
    const activeRolesStr = this.activeRoles.join(', ');
    await this.callGemini({
      model: "gemini-3-flash-preview",
      systemInstruction: AGENT_PROMPTS['Orchestrator'],
      contents: this.buildContents(`Sohbet geçmişine ve dokümanın durumuna göre bir sonraki ajanı belirle. Aktif ajanlar şunlardır: ${activeRolesStr}. Sadece bu aktif ajanlardan birini seçebilirsin. Eğer tartışılacak bir şey kalmadıysa DONE seç. Eğer insana (kullanıcıya) soru sorulması gerekiyorsa USER seç.`),
    ...
    try {
      const cleanJson = decisionJson.replace(/```json\n?|\n?```/g, '').trim();
      decisionResponse = JSON.parse(cleanJson);
    } catch(e) {
      console.error("Orchestrator decision parse error", e, decisionJson);
    }
    ...
    if (nextAgent === 'DONE' || nextAgent === 'USER') {
      return {
        ...
        finalText: decisionResponse.messageToUser || decisionResponse.reason,
        actionSummary: decisionResponse.reason
      };
    }
