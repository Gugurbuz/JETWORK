...
export function decideBaDiscovery(input: {
  ...
  const questions = buildBaClarifyingQuestions(state, 4);
...
export function evaluateBaQualityV2(document: DocumentData | null | undefined): BaQualityReportV2 {
  ...
  const sectionScores = [
    scoreSection('Keşif ve bağlam', [
      { label: 'Problem / ihtiyaç açık değil', ok: hasAny(all, [/problem/i, /ihtiya[çc]/i, /sorun/i]) },
  ...
  return {
    ...
    canPublish,
    summary: canPublish
      ? 'BA dokümanı taslak olarak paylaşılabilir; kalan maddeler iyileştirme notu seviyesinde.'
      : 'BA dokümanı henüz karar verilebilir seviyede değil; kritik keşif ve kalite eksikleri var.',
    sectionScores,
